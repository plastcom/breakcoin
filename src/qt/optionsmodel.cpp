...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeBREAmount"))
        settings.setValue("nAnonymizeBREAmount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeBREAmount = settings.value("nAnonymizeBREAmount").toLongLong();
    ...
    if (settings.contains("nAnonymizeBREAmount"))
        SoftSetArg("-anonymizeBreakCoinamount", settings.value("nAnonymizeBREAmount").toString().toStdString());
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeBREAmount:
            nAnonymizeBREAmount = value.toInt();
            settings.setValue("nAnonymizeBREAmount", nAnonymizeBREAmount);
            emit anonymizeBREAmountChanged(nAnonymizeBREAmount);
            break;
        default:
            break;
        }
