...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("BreakCoin"))
        return false;
...
{
    // Convert BreakCoin:// to BreakCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("BreakCoin://"))
    {
        uri.replace(0, 12, "BreakCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "BreakCoin.lnk";
}
...
{
    return GetAutostartDir() / "BreakCoin.desktop";
}

bool GetStartOnSystemStartup()
{
    boost::filesystem::ifstream BreakCoinFile(GetAutostartFilePath());
    if (!BreakCoinFile.good())
        return false;
    // Scan through file for "Hidden=true":
    std::string line;
    while (!BreakCoinFile.eof())
    {
        getline(BreakCoinFile, line);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        boost::filesystem::ofstream BreakCoinFile(GetAutostartFilePath(), std::ios_base::out|std::ios_base::trunc);
        if (!BreakCoinFile.good())
            return false;
        // Write a bitcoin.desktop file to the autostart directory:
        BreakCoinFile << "[Desktop Entry]\n";
        BreakCoinFile << "Type=Application\n";
        BreakCoinFile << "Name=BreakCoin\n";
        BreakCoinFile << "Exec=" << pszExePath << " -min\n";
        BreakCoinFile << "Terminal=false\n";
        BreakCoinFile << "Hidden=false\n";
        BreakCoinFile.close();
    }
    return true;
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("BreakCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  BreakCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("BreakCoin-Qt"));
