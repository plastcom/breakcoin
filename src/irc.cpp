...
void ThreadIRCSeed()
{
    ...
    RenameThread("BreakCoin-ircseed");
...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #BreakCoincoinTEST\r");
            Send(hSocket, "WHO #BreakCoincoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #BreakCoincoin\n");
            // randomly join #BreakCoincoin00-#BreakCoincoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #BreakCoincoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #BreakCoincoin%02d\r", channel_number).c_str());
        };
