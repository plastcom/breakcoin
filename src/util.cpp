...
#else
    const char* pszModule = "BreakCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BreakCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BreakCoin
    // Mac: ~/Library/Application Support/BreakCoin
    // Unix: ~/.BreakCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BreakCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "BreakCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "BreakCoind.pid"));
